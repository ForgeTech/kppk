...
function _applyVerticalSliceToPage(rows: Row[], options?: SliceOptions) {
    ...
    const verticalSlices = options?.verticalSlices ?? DefaultSliceOptions.verticalSlices as number;
    ...
    return rows.map((row) => {
        ...
        let itemIdx = 0;
        for (const [colStart, colEnd] of colXs) {
...
export function applySlice(rows: Row[], options?: SliceOptions) {
    ...
    const pages: {page: number, rows: Row[]}[] =
        rows.reduce((prev, row) => {
            let current = prev[prev.length-1] as any;
    ...
    return pages.reduce((prev, {rows}) => {
        const sliced = _applyVerticalSliceToPage(rows) as any;
