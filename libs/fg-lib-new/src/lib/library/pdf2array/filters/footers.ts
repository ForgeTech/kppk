...
function _cmpFooters(a: Row, b: Row, options?: StripFootersOptions) {
    ...
    const yTolerance = options?.yTolerance ?? DefaultFooterOptions.yTolerance as number;
    if (Math.abs(a.y - b.y) > yTolerance) {
        return false;
    }

    const xTolerance = options?.xTolerance ?? DefaultFooterOptions.xTolerance as number;
...
export function stripFooters(rows: Row[], options?: StripFootersOptions) {
    ...
    if (largestCluster !== undefined) {
        if ((largestCluster[1] / footers.length) > (options?.confidence ?? DefaultFooterOptions.confidence as number)) {
