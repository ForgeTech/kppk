...
export function stripSuperscripts(rows: Row[], options?: StripSuperscriptOptions) {
    const radiusScale = options?.radiusScale ?? DefaultOptions.radiusScale as number;
    const heightScale = options?.heightScale ?? DefaultOptions.heightScale as number;
    const stripLeft = options?.stripLeft ?? DefaultOptions.stripLeft as boolean;
    const stripRight = options?.stripRight ?? DefaultOptions.stripRight as boolean;
