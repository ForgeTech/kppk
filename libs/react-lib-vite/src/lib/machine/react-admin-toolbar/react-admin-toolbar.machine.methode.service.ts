...
export class ReactAdminToolbarMachineMethodeService extends FgBaseService {
  ...
  // }
  @boundMethod
  public assign_update_context_from_main_auth_actor({
    context,
    event,
    system,
  }: {
  ...
  @boundMethod
  public guard_env_is_dev_enabled({
    context,
    event,
  }: {
    context: REACT_ADMIN_TOOLBAR_CONTEXT;
  ...
  public guard_user_is_admin({
    ...
    const result = this.$actor_auth.stateS()?.context?.auth_cookie?.profile
      ?.admin
      ? true
      : false;
    return result;
