...
export const power_supply_calculate_energy_usage_estimate = (
  ...
  const value =
    gross_floor_area.value * operation_period.value * estimate_value.value;
  return unit_kilowatthours_parser.parse({ value });
};
...
export const calculate_construction_site_results = (
  ...
  if (
    form_construction_site.value.energy_usage_settings.energy_usage_build_type
      .value === BUILD_TYPE_ENUM.building_construction
  ) {
    let year_energy_values;
    switch (
      form_construction_site.value.energy_usage_settings
        .energy_usage_calculation_type.value
    ) {
      case POWER_SUPPLY_CALCULATION_TYPE_ENUM.estimate:
        result.power_supply.energy_usage =
          power_supply_calculate_energy_usage_estimate(
      ...
      case POWER_SUPPLY_CALCULATION_TYPE_ENUM.exact_entry:
        year_energy_values =
          form_construction_site.value.energy_usage_values.year_energy_usage.map(
            (year) => {
        ...
        result.power_supply.energy_usage =
          calculate_energy_usage_exact_entry(year_energy_values);
        break;
    ...
    switch (
      form_construction_site.value.heating_supply_settings.calculation_type
        .value
    ) {
      case HEAT_SUPPLY_CALCULATION_TYPE_ENUM.estimate:
        result.heating_supply.energy_usage = kwh_to_mwh(
          power_supply_calculate_energy_usage_estimate(
      ...
      case HEAT_SUPPLY_CALCULATION_TYPE_ENUM.exact_entry:
        year_energy_values =
          form_construction_site.value.heating_supply_values.year_energy_usage.map(
            (year) => {
              return Object.entries(year).map((item) => {
                const [key, value] = item;
                return value;
      ...
      case HEAT_SUPPLY_CALCULATION_TYPE_ENUM.custom:
        result.heating_supply.energy_usage =
          heating_supply_calculate_energy_usage(
            form_construction_site.value.heating_supply_values.operation_period,
            form_construction_site.value.heating_supply_values
              .energy_usage_custom
          );
        break;
      default:
    ...
    const power_supply_energy_type = data.material_co2_equ.find(
      (item) =>
        item.material.value ===
        form_construction_site.value.energy_usage_settings
          .energy_usage_power_type.value
    );
    const heating_supply_power_type = data.material_co2_equ.find(
      (item) =>
        item.material.value ===
        form_construction_site.value.heating_supply_settings
          .energy_usage_power_type.value
    );

    if (power_supply_energy_type && heating_supply_power_type) {
      const power_supply_energy_co2_equ =
        unit_kilogramco2_kilowatthour_parser.parse(
          power_supply_energy_type.co2_equ
        );
      result.power_supply.co2_energy_usage =
        calculate_energy_usage_estimate_co2(
          result.power_supply.energy_usage,
          power_supply_energy_co2_equ
        );

      const heating_supply_energy_co2_equ =
        unit_kilogramco2_kilowatthour_parser.parse(
          heating_supply_power_type.co2_equ
        );
      result.heating_supply.co2_energy_usage =
        calculate_energy_usage_estimate_co2(
      ...
      throw new Error(
        'ERROR_CALCULATION_EXCAVATION_PIT_REQUIRED_INPUT_MISSING'
      );
    }
    const heating_supply_fuel_oil = data.material_co2_equ.find(
      (item) => item.material.value === 'fuel_oil'
    );
    if (heating_supply_fuel_oil) {
      const co2_kg_fuel_oil = unit_kilogramco2_liter_parser.parse(
        heating_supply_fuel_oil.co2_equ
      );
      result.heating_supply.fuel_oil_usage =
        form_construction_site.value.heating_supply_values.fuel_oil_usage;
    ...
    result.co2_heating_supply = add_number_units(
      result.heating_supply.co2_energy_usage,
      result.heating_supply.co2_fuel_oil_usage
    );

    result.co2_supply = add_number_units(
      result.power_supply.co2_energy_usage,
      result.co2_heating_supply
    );
  } else {
