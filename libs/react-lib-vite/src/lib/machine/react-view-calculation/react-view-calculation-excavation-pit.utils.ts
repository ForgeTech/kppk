...
export const calculate_shotcrete_additional_excavation = (
  ...
  const value =
    depth.value * linear_meter.value * thickness.value * density.value;
  return unit_kilogram_parser.parse({ value });
};
...
export const degrees_to_radiants = (degrees: UNIT_DEGREE) => {
...
export const double_co2 = (truck_co2_km: UNIT_KGCO2_KM): UNIT_KGCO2_KM => {
...
export const calculate_building_gap_linear_meter = (
  linear_meter: UNIT_M
): UNIT_M => {
...
export const calculate_shotcrete_co2_nail_volumen = (
  nail_count: UNIT_PIECES,
  nail_length: UNIT_M,
  nail_diameter: UNIT_MM
): UNIT_M3 => {
...
export const calculate_excavation_pit_results = (
  ...
  const concrete_truck = truck_data_item_parser.parse(
    data.truck.find((item) => item.category.includes('concrete'))
  );
  const excavation_truck = truck_data_item_parser.parse(
    data.truck.find((item) => item.category.includes('excavation'))
  );

  const concrete_density = material_density_item_parser.parse(
    data.material_density.find((item) => item.material.value === 'concrete')
  );
  const excavation_density = material_density_item_parser.parse(
    data.material_density.find((item) => item.material.value === 'excavation')
  );
  const steel_density = material_density_item_parser.parse(
    data.material_density.find((item) => item.material.value === 'steel')
  );

  const concrete_co2_equ_item = material_co2_equ_item_parser.parse(
    data.material_co2_equ.find((item) => item.material.value === 'concrete')
  );
  const concrete_co2_kg = unit_kilogramco2_kilogram_parser.parse(
    concrete_co2_equ_item.co2_equ
  );

  const steel_co2_equ_item = material_co2_equ_item_parser.parse(
    data.material_co2_equ.find((item) => item.material.value === 'steel')
  );
  const steel_co2_kg = unit_kilogramco2_kilogram_parser.parse(
    steel_co2_equ_item.co2_equ
  );

  let linear_meter =
    form_excavation_pit.value.excavation_pit_security.linear_meter;
  ...
  // );
  const jet_blasting_process_cylinder_concrete_volume =
    calculate_jet_blasting_process_cylinder_volume(
      form_excavation_pit.value.excavation_pit_security.depth,
      form_excavation_pit.value.jet_blasting_process
        .jet_blasting_process_cylinder.diameter,
      form_excavation_pit.value.jet_blasting_process.amount,
      form_excavation_pit.value.jet_blasting_process
        .jet_blasting_process_cylinder.shape
    );
  ...
  const jet_blasting_process_cylinder_concrete_mass_co2_transport =
    co2_transport(
  ...
  const jet_blasting_process_cuboid_concrete_volume =
    calculate_jet_blasting_process_cubid_volume(
      form_excavation_pit.value.excavation_pit_security.depth,
      form_excavation_pit.value.jet_blasting_process.jet_blasting_process_cuboid
        .length,
      form_excavation_pit.value.jet_blasting_process.jet_blasting_process_cuboid
        .width
    );
  ...
  if (
    form_excavation_pit.value.jet_blasting_process.process_type.value ===
    'process_type_cylinder'
  ) {
    concrete_mass_result = jet_blasting_process_cylinder_concrete_mass;
    co2_transport_result =
      jet_blasting_process_cylinder_concrete_mass_co2_transport;
    co2_creation_result =
      jet_blasting_process_cylinder_concrete_mass_co2_creation;
  } else if (
    form_excavation_pit.value.jet_blasting_process.process_type.value ===
    'process_type_cuboid'
  ) {
    ...
    co2_transport_result =
      jet_blasting_process_cuboid_concrete_mass_co2_transport;
    co2_creation_result =
      jet_blasting_process_cuboid_concrete_mass_co2_creation;
  } else {
    throw new Error(
      'ERROR_EXCAVATION_PIT_SECURITY_JET_BLASTING_METHODE_NOT_FOUND'
    );
  }
  const escarpment_additional_excavation =
    calculate_escarpment_additional_excavation(
      form_excavation_pit.value.excavation_pit_security.depth,
      linear_meter,
      excavation_density.density,
      form_excavation_pit.value.escarpment.tilt
    );
  const escarpment_co2_transport = double_co2(
    co2_transport(
  ...
  const sheet_pile_wall_co2_transport = double_co2(
    co2_transport(
  ...
  const shotcrete_additional_excavation =
    calculate_shotcrete_additional_excavation(
      // E3
      form_excavation_pit.value.excavation_pit_security.depth,
      // *E4
      linear_meter,
  ...
  const shotcrete_escarpment_additional_excavation =
    calculate_escarpment_additional_excavation(
      form_excavation_pit.value.excavation_pit_security.depth,
      linear_meter,
      excavation_density.density,
  ...
  const shotcrete_escarpment_co2_transport = double_co2(
    co2_transport(
  ...
  const shotcrete_co2_transport = add_number_units(
    shotcrete_additional_excavation_co2_transport,
    shotcrete_escarpment_co2_transport
  );
  ...
  const steel_volumn_mass = unit_kilogram_parser.parse({
    value: steel_volumn.value * steel_density.density.value,
  });
  ...
  // =((E11^2*PI())/4)*E3*E12
  const foundation_pile_concrete_volumn =
    calculate_foundation_pile_concrete_volumne(
      // E11
  ...
  switch (form_excavation_pit.value.excavation_pit_security.methode.value) {
    case EXCAVATION_PIT_SECURITY_METHODE_ENUM.escarpment:
      excavation_pit_security_co2_transport =
        calculate_excavation_pit_security_co2_transport(
