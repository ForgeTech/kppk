...
export function matchValidator(
  ...
  if (
    field.props &&
    field.props['matchSource'] &&
    field?.props['matchTarget']
  ) {
...
export function isWindowPartTypeValidator(control: AbstractControl) {
  const result =
    control.value === '-' ? { requires_window_part_type: true } : null;
  return result;
}
...
@NgModule({
  imports: [
    ...
    FormlyModule.forRoot({
      ...
      types: [
        {
          name: 'input-number-digit',
          component: FgFormlyFieldNumberDigitInputComponent,
        },
        { name: 'input-file', component: FgFormlyFieldFile },
        {
          name: 'energy-usage-yearly-array',
          component: KppkEnergyUsageYearlyArrayTypeComponent,
        },
      ],
      validators: [
...
export class KppkFormlyModule {
  ...
  constructor() {
    this.$config.addValidatorMessage(
      'required',
      this.formly_validation_message_error_required
    );
    this.$config.addValidatorMessage(
      'isWindowPartType',
      this.formly_validation_message_is_window_part_type
    );
  }
