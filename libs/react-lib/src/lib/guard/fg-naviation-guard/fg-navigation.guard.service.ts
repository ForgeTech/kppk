...
import { FgNavigationMachineActorService } from '../../machine';
...
export class FgNavigationGuard implements CanActivate, CanDeactivate<any> {


    protected $actor_navigation = inject(FgNavigationMachineActorService);
    ...
    protected disallow_navigation_for_router_disabled$ = this.$actor_navigation.state$.pipe( 
      filter( snapshot => snapshot.matches({'SETTINGS': 'DISABLED'})),
      map( _ => {
        return false;
      }
      )
    ...
    protected resolve_for_navigation_for_router_blocked$(component: any, currentRoute?: ActivatedRouteSnapshot, currentState?: RouterStateSnapshot, nextState?: RouterStateSnapshot ) {
      return this.$actor_navigation.state$.pipe(
        filter( 
          snapshot => snapshot.matches({'SETTINGS': 'BLOCKED'}) 
          && snapshot.matches({'SETTINGS': {'BLOCKED': 'IDEL'}}) === false 
        ),
        switchMap( snapshot => {
          // if routing is in request permission state 
          if(snapshot.matches({'SETTINGS': { 'BLOCKED': 'REQUEST_PERMISSION'}})) {
            ...
            if(snapshot.matches({'SETTINGS': { 'BLOCKED': 'ACCEPTED'}})) {
              // If confirm dialog
              return of(true);
            } else if(snapshot.matches({'SETTINGS': { 'BLOCKED': 'DECLINED'}})) {
              return of(false);
            } else {
              throw new Error(
    ...
    protected resolve_navigation_for_router_enabled$ = this.$actor_navigation.state$.pipe( 
      filter( snapshot => 
        // If normal routing is enabled
        snapshot.matches({'SETTINGS': 'ENABLED'}) 
        // wait for navigation to reach state allow or redirect
        // && ( 
          // snapshot.matches({'NAVIGATION': 'RU'}) 
          // || snapshot.matches({'NAVIGATION': 'REDIRECT'}) 
        // ) 
      ),
      map( snapshot => {
        if( snapshot.matches({'SETTINGS': 'ENABLED'}) ) {
          // allow navigation
          return true;
        } 
    ...
    protected send_permission_request_answer_event(accept_or_declined: boolean) {
      if( accept_or_declined === true) {
        this.$actor_navigation.send({ type: 'fg.navigation.event.permission_request.accept'})
      } else {
        this.$actor_navigation.send({ type: 'fg.navigation.event.permission_request.decline'})
      }
    }
