...
import {
  COMPONENT_SURFACE_AREA_TRANSFORM_STEP_1,
  MATERIAL_OI3_STEP_2,
} from '../../types';

const transform_comma_seperated_numberstring_to_number = (
  stringNumberToParse: string
): number => {
  // console.log('stringNumberToParse')
  if (
    stringNumberToParse === '' ||
    stringNumberToParse === null ||
    stringNumberToParse === undefined
  ) {
    return NaN;
  }
...
export const merge_bauteilflaechen_aufbauten = (
  transformed_aufbauten: any[],
  transformed_bauteilflaechen: any[]
) => {
  const join_bauteilflaechen_plus_aufbauten_helper: {
    [key: string]: COMPONENT_SURFACE_AREA_TRANSFORM_STEP_1 | undefined;
  } = {};
  ...
  const result: any[] = transformed_aufbauten
    .filter((row_aufbauten) => {
      const found_bauteilflaechen_row = transformed_bauteilflaechen.find(
        (row_bauteilflaechen) =>
          row_aufbauten.short_id.value === row_bauteilflaechen.short_id.value
      );
      if (found_bauteilflaechen_row) {
        join_bauteilflaechen_plus_aufbauten_helper[
          row_aufbauten.short_id.value
        ] = found_bauteilflaechen_row;
        return true;
      } else {
        return false;
      }
    ...
    .map((row_aufbauten) => {
      ...
      if (row_bauteilflaechen) {
        const area_object = {
          area: {
            value:
              row_bauteilflaechen.area_sum.value *
              row_aufbauten.area_coefficient.value,
            unit: 'm²',
          },
        };
    ...
    .map((row_aufbauten) => {
      const mass = {
        mass: {
          value:
            row_aufbauten.surface_related_mass.value * row_aufbauten.area.value,
          unit: '',
        },
      };
...
export const transformed_arich_plus_oi3_source_1 = (transformed_oi3: any[]) => {
  ...
  const result: any[] = transformed_oi3
    ...
    .reduce(
      ...
      [{}] as {
        [key: string]: {
          gwp_total: { value: number; unit: '' };
          area: { value: number; unit: 'm2' };
        };
      }[]
    )
    .map((reduced: any) => {
...
export const transformed_arich_plus_oi3_source_2 = (
  merged_bauteilflaechen_plus_aufbauten: any[],
  transformed_oi3: any[]
) => {
  const join_arich_plus_oi3_helper: {
    [key: string]: MATERIAL_OI3_STEP_2 | undefined;
  } = {};
  const result = merged_bauteilflaechen_plus_aufbauten
    .filter((row_arich_plus_oi3_source_2) => {
      ...
      const found = transformed_oi3.find(
        (item) => row_arich_plus_oi3_source_2.name.value === item.name.value
      );
      if (found) {
        join_arich_plus_oi3_helper[row_arich_plus_oi3_source_2.name.value] =
          found;
        return true;
      } else {
        return false;
...
export const transformed_arich_plus_oi3_source_2_found = (
  transformed_arich_plus_oi3_source_1: any[],
  transformed_arich_plus_oi3_source_2: any[]
) => {
  const result = transformed_arich_plus_oi3_source_2
    ...
    .map((row) => {
      ...
      const test = transformed_arich_plus_oi3_source_1.find(
        (row_source_1) => row.name.value === row_source_1.name.value
      );
      let gwp = io3.value?.gwp_total.value;
      if (test && io3.value?.gwp_total.value) {
        const changed =
          Math.abs(test.gwp_total.value) > Math.abs(io3.value.gwp_total.value);
        if (changed) {
          // console.log('CHANGED');
...
export const transformed_arich_plus_oi3_source_2_not_found = (
  transformed_arich_plus_oi3_source_2: any[]
) => {
  const join_arich_plus_oi3_not_found_helper: {
    [key: string]: MATERIAL_OI3_STEP_2 | undefined;
  } = {};
  const result = transformed_arich_plus_oi3_source_2
    // FEHLER! Vollholzgurt: Nutzholz (475 kg/m³ - zB Fichte/Tanne) - rauh, technisch getrockne vorhanden
    ...
    .map((row_arich_plus_oi3_source_2) => {
      const row_oi3 =
        join_arich_plus_oi3_not_found_helper[
          row_arich_plus_oi3_source_2.name.value
        ];
      if (row_oi3) {
        const joing_object = {
          table_oi3: {
...
export const transformed_arich_plus_oi3_source_2_o1 = (
  transformed_oi3: any[],
  transformed_bauteilflaechen_plus_aufbauten: any[]
) => {
  ...
  const result = nestedLoopLeftAntiJoin(
    ...
    transformed_bauteilflaechen_plus_aufbauten,
    (obj) => {
      return obj.name.value;
    }
...
// })
export const concrete_items = (
  transformed_arich_plus_oi3_source_2_found: any[],
  transformed_arich_plus_oi3_source_2_not_found: any[]
) => {
  const result = [
...
export const material_items = (
  ...
  const result = [
    ...
    .filter((item) => {
      return (
        item.name.value.toLowerCase().indexOf('beton') === -1 &&
        item.name.value.toLowerCase().indexOf('stb-') === -1
      );
    })
    .map((item) => {
      const type_object = {
