...
export const get_section_result = (
  ...
  Object.keys(section).forEach((key) => {
    const material = demolish_disposal_form_material_item_parser.parse(
      section_helper[key]
    );
    const material_density = material_density_item_parser.parse(
      density_data.find((entry) => entry.material.value === key)
    );
    const truck = truck_data_item_parser.parse(
      truck_data.find((truck) =>
        truck.category.includes(material.container.value)
      )
    );
    ...
    if (truck.max_density.value < material_density.density.value) {
      const material_mass = calculate_mass(
        material.volume,
        material_density.density
      );
      truck_capacity = calculate_weight_capacity_from_truck_usage(
    ...
    // calculate of volume capacity
      ...
      truck_capacity = calculate_volume_capacity_from_truck_usage(
        truck.capacity_volume,
        setting.usage
      );
      trips = calculate_trips_from_volume(material.volume, truck_capacity);
    }
    const distance_sum = calculate_distance_from_trips(
      material.distance,
      trips
    );
    const consumption_sum = calculate_consumption_sum(
      distance_sum,
      truck.consumption
    );
    // consumption_sum_wrong = this.calculatConsumptionSum( material.distance, truck.consumption);
    const consumption_co2 = calculate_consumption_co2(
      distance_sum,
      truck.co2_consumption
    );
...
export const calculate_demolish_disposal_results = (
  ...
  const consumption_co2_sum = section_items
    .map((value) => value.consumption_co2)
    .reduce((sum, kg_co2) => {
