...
import {
  ...
  ROSE_FILE_DATA,
} from '../../types/kppk-react-heating-system.types';
import {
  UNIT_TCO2,
  UNIT_TCO2_YEAR,
  unit_tonco2_parser,
...
export const calculate_usage_co2 = (
  system_co2_year: UNIT_TCO2_YEAR,
  calc_usage: UNIT_YEAR
): UNIT_TCO2 => {
  const value = system_co2_year.value * calc_usage.value;
  return unit_tonco2_parser.parse({ value });
};

export const form_heating_system_calculate_dynamic_model_values = (
  form_heating_system: FORM_HEATING_SYSTEM_DATA,
  rose_file_data: ROSE_FILE_DATA
): FORM_HEATING_SYSTEM_DATA => {
  const result = form_heating_system_data_parser.parse(form_heating_system);
  let found_item =
    rose_file_data[
      form_heating_system.system_select.value as keyof typeof rose_file_data
    ];
  if (!found_item) {
...
export const calculate_heating_system_results = (
  ...
  const result: FORM_HEATING_SYSTEM_RESULT = {
    calc_usage_co2: calculate_usage_co2(
      form_heating_system.value.system_co2_year,
      form_heating_system.value.calc_usage
    ),
  };
