...
export class ReactViewCalculationMachineMethodeService extends FgBaseService {
  ...
  public actor_merge_bauteilflaechen_aufbauten = async ({
    input,
  }: {
    ...
    if (
      input.context.calculation?.actor_transform_file_inputs
        ?.transformed_aufbauten &&
      input.context.calculation?.actor_transform_file_inputs
        .transformed_bauteilflaechen
    ) {
      const transformed_aufbauten = cloneDeep(
        input.context.calculation?.actor_transform_file_inputs
          .transformed_aufbauten
      );
      const transformed_bauteilflaechen = cloneDeep(
        input.context.calculation?.actor_transform_file_inputs
          .transformed_bauteilflaechen
      );
      merged_bauteilflaechen_aufbauten = merge_bauteilflaechen_aufbauten(
        transformed_aufbauten,
        transformed_bauteilflaechen
      );
    }
  ...
  public actor_merge_arich_oi3 = async ({
    input,
  }: {
    input: REACT_VIEW_CALCULATION_INPUT;
  }) => {
    ...
    if (
      input.context.calculation?.actor_transform_file_inputs &&
      input.context.calculation.actor_merge_bauteilflaechen_aufbauten
        .merged_bauteilflaechen_aufbauten
    ) {
      const transformed_oi3 = cloneDeep(
        input.context.calculation.actor_transform_file_inputs.transformed_oi3
      );
      const merged_bauteilflaechen_aufbauten = cloneDeep(
        input.context.calculation.actor_merge_bauteilflaechen_aufbauten
          .merged_bauteilflaechen_aufbauten
      );
      result.transformed_arich_plus_oi3_source_1 =
        transformed_arich_plus_oi3_source_1(transformed_oi3);
      result.transformed_arich_plus_oi3_source_2 =
        transformed_arich_plus_oi3_source_2(
          merged_bauteilflaechen_aufbauten,
          transformed_oi3
        );

      result.transformed_arich_plus_oi3_source_2_found =
        transformed_arich_plus_oi3_source_2_found(
          result.transformed_arich_plus_oi3_source_1,
          result.transformed_arich_plus_oi3_source_2
        );
      result.transformed_arich_plus_oi3_source_2_not_found =
        transformed_arich_plus_oi3_source_2_not_found(
          result.transformed_arich_plus_oi3_source_2
        );
      result.transformed_arich_plus_oi3_source_2_o1 =
        transformed_arich_plus_oi3_source_2_o1(
          transformed_oi3,
          merged_bauteilflaechen_aufbauten
        );
      const window_category_items: any = window_items(
        transformed_oi3,
        merged_bauteilflaechen_aufbauten
      );
      const concrete_category_items: any = concrete_items(
        result.transformed_arich_plus_oi3_source_2_found,
        result.transformed_arich_plus_oi3_source_2_not_found
      );
      const material_category_items: any = material_items(
        result.transformed_arich_plus_oi3_source_2_found,
        result.transformed_arich_plus_oi3_source_2_not_found
      );
  ...
  public actor_prepare_material_types = async ({
    input,
  }: {
    input: REACT_VIEW_CALCULATION_INPUT;
  }) => {
    ...
    if (
      input.context.calculation &&
      input.context.calculation.actor_merge_arich_oi3?.material_type
    ) {
      ...
      material_types.forEach((item, index) => {
        ...
        switch (item.type.value) {
          case 'concrete':
            ...
            concrete_item = this.calculate_concrete_item(
              concrete_item,
              input.context
            );
          ...
          case 'window':
            ...
            if (windows && frames) {
              if (name.indexOf('glas') !== -1 || name.indexOf('glass') !== -1) {
                window_item.window_part_type.value =
                  WINDOW_PART_TYPE_ENUM.glass as string;
              } else if (
                name.indexOf('rahmen') !== -1 ||
                name.indexOf('frame') !== -1
              ) {
                window_item.window_part_type.value =
                  WINDOW_PART_TYPE_ENUM.frame as string;
              } else {
                window_item.window_part_type.value =
                  WINDOW_PART_TYPE_ENUM.none as string;
              }
            }
          ...
          case 'material':
            ...
            material_item = this.calculate_material_item(
              material_item,
              input.context
            );
  ...
  public actor_concrete_calculation = async ({
    ...
    const form = form_concrete_context_parser.parse(
      input.context.calculation?.form_concrete
    );
  ...
  public actor_window_calculation = async ({
    ...
    const form = form_window_context_parser.parse(
      input.context.calculation?.form_window
    );
  ...
  public actor_container_village_calculation = async ({
    input,
  }: {
    input: REACT_VIEW_CALCULATION_INPUT;
  }) => {
    let result = {};
    if (
      input.context.calculation?.form_container_village &&
      input.context.data
    ) {
      result = calculate_container_village_results(
        input.context.calculation.form_container_village,
        input.context.data
      );
    }
  ...
  public actor_construction_site_calculation = async ({
    ...
    let result = {};
    if (
      input.context.calculation?.form_construction_site &&
      input.context.data
    ) {
      result = calculate_construction_site_results(
        input.context.calculation.form_construction_site,
        input.context.data
      );
    }
  ...
  public actor_demolish_disposal_calculation = async ({
    ...
    if (
      input.context.calculation?.form_demolish_disposal &&
      input.context.data
    ) {
      result = calculate_demolish_disposal_results(
        input.context.calculation.form_demolish_disposal,
        input.context.data
      );
    }
  ...
  public actor_excavation_pit_calculation = async ({
    ...
    if (input.context.calculation?.form_excavation_pit && input.context.data) {
      result = calculate_excavation_pit_results(
        input.context.calculation.form_excavation_pit,
        input.context.data
      );
    }
    return result;
  ...
  public actor_heating_system_calculation = async ({
    input,
  }: {
    input: REACT_VIEW_CALCULATION_INPUT;
  }) => {
    ...
    if (input.context.calculation?.form_excavation_pit && input.context.data) {
      result = calculate_heating_system_results(
        input.context.calculation.form_heating_system,
        input.context.data
      );
    }
  ...
  public actor_result_calculation = async ({
    input,
  }: {
    input: REACT_VIEW_CALCULATION_INPUT;
  }) => {
    if (input.context.calculation) {
      ...
      console.log(
        JSON.stringify(input.context.calculation.form_heating_system)
      );
    }
  ...
  public assign_common_form_data = ({
    context,
    event,
  }: REACT_VIEW_CALCULATION_INPUT) => {
  ...
  public assign_result_calculation_result = ({
    context,
    event,
  }: REACT_VIEW_CALCULATION_INPUT) => {
    const result = this.$immer.produce(context, (draft) => {
  ...
  public assign_heating_system_form_data = ({
    context,
    event,
  }: REACT_VIEW_CALCULATION_INPUT) => {
    const result = this.$immer.produce(context, (draft) => {
      if (context.calculation && draft.calculation) {
        ...
        if (event.payload.valid) {
          const update_dynamic_values =
            form_heating_system_calculate_dynamic_model_values(
              event.payload.data,
              context.calculation.file_rose
            );
  ...
  public assign_transformed_file_inputs = ({
    context,
    event,
  }: REACT_VIEW_CALCULATION_INPUT) => {
    const result = this.$immer.produce(context, (draft) => {
  ...
  public assign_change_aufbauten = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.actor_transform_file_inputs
          ?.transformed_aufbauten &&
        draft?.calculation?.actor_transform_file_inputs?.transformed_aufbauten
      ) {
        draft.calculation.actor_transform_file_inputs.transformed_aufbauten[
          event.payload.index
        ] = event.payload.data;
  ...
  public assign_change_bauteilflaechen = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.actor_transform_file_inputs
          ?.transformed_bauteilflaechen &&
        draft?.calculation?.actor_transform_file_inputs
          ?.transformed_bauteilflaechen
      ) {
        draft.calculation.actor_transform_file_inputs.transformed_bauteilflaechen[
          event.payload.index
        ] = event.payload.data;
      }
    });
  ...
  public assign_change_oi3 = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.actor_transform_file_inputs?.transformed_oi3 &&
        draft?.calculation?.actor_transform_file_inputs?.transformed_oi3
      ) {
        draft.calculation.actor_transform_file_inputs.transformed_oi3[
          event.payload.index
        ] = event.payload.data;
  ...
  public assign_prepare_material_types = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (context.calculation && draft.calculation) {
        ...
        draft.calculation.form_material.value.rows =
          event.output.material_items;
        draft.calculation.form_concrete.value.rows =
          event.output.concrete_items;
  ...
  public assign_change_material_calculation = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.form_material.value.rows &&
        draft?.calculation
      ) {
        ...
        // )
        draft.calculation.form_material.value.rows[event.payload.index] =
          this.calculate_material_item(item, context);
      }
    });
    return result;
  };
  ...
  public assign_change_window_calculation = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (context?.calculation?.form_window.value.rows && draft?.calculation) {
        try {
          const prev_item: FORM_WINDOW_VALUE =
            context.calculation.form_window.value.rows[event.payload.index];
          ...
          if (
            prev_item.window_part_type.value !== payload.window_part_type.value
          ) {
          ...
          draft.calculation.form_window.value.rows[event.payload.index] =
            this.calculate_window_item(window_item, context);
        } catch (error) {
          console.log(error);
        }
      }
  ...
  public assign_change_concrete_calculation = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.form_concrete.value.rows &&
        draft?.calculation
      ) {
        const item = form_concrete_value_parser.parse(event.payload.data);
        draft.calculation.form_concrete.value.rows[event.payload.index] =
          this.calculate_concrete_item(item, context);
      }
    });
    return result;
  };
  ...
  public assign_container_village_calculation_result = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.form_container_village.value &&
        draft?.calculation
      ) {
  ...
  public assign_construction_site_calculation_result = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.form_construction_site.value &&
        draft?.calculation
      ) {
  ...
  public assign_demolish_disposal_calculation_result = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.form_demolish_disposal.value &&
        draft?.calculation
      ) {
  ...
  public assign_excavation_pit_calculation_result = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.form_excavation_pit.value &&
        draft?.calculation
      ) {
  ...
  public assign_heating_system_calculation_result = ({
    context,
    event,
  }: REACT_VIEW_CALCULATION_INPUT) => {
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.form_heating_system.value &&
        draft?.calculation
      ) {
  ...
  public assign_change_material_type = ({
    ...
    const result = this.$immer.produce(context, (draft) => {
      if (
        context?.calculation?.actor_merge_arich_oi3?.material_type &&
        draft?.calculation?.actor_merge_arich_oi3?.material_type
      ) {
        draft.calculation.actor_merge_arich_oi3.material_type[
          event.payload.index
        ] = event.payload.data;
  ...
  public raise_form_internal_update = ({
    context,
    event,
  }: REACT_VIEW_CALCULATION_INPUT) => {
  ...
  public guard_container_village_form_is_selected = ({
    context,
  }: REACT_VIEW_CALCULATION_INPUT) => {
  ...
  public guard_merge_bauteilflaechen_aufbauten_data_exists = ({
    ...
    const result = context.calculation?.actor_merge_bauteilflaechen_aufbauten
      ?.merged_bauteilflaechen_aufbauten
      ? true
      : false;
    return result;
  ...
  public guard_materials_with_type_data_exists = ({
    ...
    const result = context.calculation?.actor_merge_arich_oi3?.material_type
      ? true
      : false;
    return result;
  };
  ...
  public guard_material_calculation_data_exist = ({
    ...
    const result = context.calculation?.actor_merge_arich_oi3?.material_items
      ? true
      : false;
    return result;
  };
  ...
  public guard_concrete_calculation_data_exist = ({
    ...
    const result = context.calculation?.actor_merge_arich_oi3?.concrete_items
      ? true
      : false;
    return result;
  };
  ...
  public guard_window_calculation_data_exist = ({
    ...
    const result = context.calculation?.actor_merge_arich_oi3?.window_items
      ? true
      : false;
    return result;
  };
  ...
  public guard_container_village_selected = ({
    ...
    const result = context.calculation?.form_step_selection.value
      .container_village
      ? true
      : false;
    return result;
  ...
  public guard_excavation_pit_selected = ({
    ...
    const result = context.calculation?.form_step_selection.value.excavation_pit
      ? true
      : false;
    return result;
  };
  ...
  public guard_heating_system_selected = ({
    ...
    const result = context.calculation?.form_step_selection.value.heating_system
      ? true
      : false;
    return result;
  };

  public calculate_shipments(
    item: FORM_MATERIAL_VALUE | FORM_CONCRETE_VALUE | FORM_WINDOW_VALUE,
    truck: TRUCK_DATA_ITEM,
    usage_in_percent: UNIT_PERCENT = {
      value: 100,
      unit: '%',
      edited: false,
      type: 'number',
  ...
  public calculate_co2_transport(
    item: FORM_MATERIAL_VALUE | FORM_CONCRETE_VALUE | FORM_WINDOW_VALUE,
    truck: TRUCK_DATA_ITEM
  ) {
    const value =
      (truck.co2_consumption.value *
        item.distance.value *
  ...
  public calculate_material_item(
    item: FORM_MATERIAL_VALUE,
    context: REACT_VIEW_CALCULATION_CONTEXT
  ) {
    ...
    if (item && truck && material_defaults) {
      item.shipments = this.calculate_shipments(
        item,
        truck,
        material_defaults.truck_usage
      );
  ...
  public calculate_concrete_item(
    item: FORM_CONCRETE_VALUE,
    context: REACT_VIEW_CALCULATION_CONTEXT
  ) {
    ...
    if (item && truck && concrete_defaults) {
      item.shipments = this.calculate_shipments(
        item,
        truck,
        concrete_defaults.truck_usage
      );
      if (item.distance.value === 0) {
        item.distance = unit_kilometer_parser.parse(concrete_defaults.distance);
      }
      const concrete = context.data?.concrete_types.find(
        (c_item) => c_item.id.value === item.gwp_oeko_id.value
      );
      if (concrete) {
  ...
  public calculate_window_item(
    item: FORM_WINDOW_VALUE,
    context: REACT_VIEW_CALCULATION_CONTEXT
  ) {
    const truck = context.data?.truck[5];
    ...
    if (item && truck && window_defaults && windows && frames) {
      ...
      if (
        item.window_part_type.value === (WINDOW_PART_TYPE_ENUM.glass as string)
      ) {
        if (item.window_part_type_glass_id === undefined) {
          item.window_part_type_glass_id = unit_id_parser.parse(windows[0].id);
        }
        item.window_part_type_glass_id = unit_id_parser.parse(windows[0].id);
        item.window_part_type_glass = windows.find(
          (window_item) =>
            window_item.id.value === item.window_part_type_glass_id?.value
        );
        item.density = item.window_part_type_glass.density;
        item.mass = this.calculate_mass(
          item.area,
          item.window_part_type_glass.area_based_mass
        );
      } else if (
        ...
        item.window_part_type_frame = frames.find(
          (frame) => frame.id.value === item.window_part_type_frame_id?.value
        );
        item.density = item.window_part_type_frame.density;
        item.mass = this.calculate_mass(
          item.area,
          item.window_part_type_frame.area_based_mass
        );
      } else {
        // Reset values
      ...
      if (
        item.window_part_type.value === WINDOW_PART_TYPE_ENUM.glass ||
        item.window_part_type.value === WINDOW_PART_TYPE_ENUM.frame
      ) {
        // console.log( item );
  ...
  public roundNumbersOnObject<T extends { [id: string]: any }>(
    item: T,
    defaults: any
  ) {
    Object.keys(item).forEach((key) => {
      try {
        ...
        if (
          item_obj?.value &&
          typeof item_obj.value === 'number' &&
          isFrozen === false
        ) {
          const result = parseFloat(item_obj.value).toFixed(
            defaults?.round.value
          );
