...
//   constext: react_calculation_materials_v1_context,
//   event: z.any()
// });
...
export class ReactCalculationMaterialService extends FgBaseService {
  ...
  public get_machine(context?: REACT_CALCULATION_MATERIALS_CONTEXT) {
    return this.$xstate
      .setup({
        types: {
        ...
        actions: {
          assign_oi3_transform_result: this.$xstate.assign(
            this.$methode.assign_oi3_transform_result
          ),
          assign_bauteilflaechen_transform_result: this.$xstate.assign(
            this.$methode.assign_bauteilflaechen_transform_result
          ),
          assign_aufbauten_transform_result: this.$xstate.assign(
            this.$methode.assign_aufbauten_transform_result
          ),
          ...
          assign_material_type: this.$xstate.assign(
            this.$methode.assign_material_type
          ),
        },
        actors: {
          actor_tranform_oi3_file_inputs: this.$xstate.fromPromise(
            this.$methode.actor_tranform_oi3_file_inputs
          ),
          actor_tranform_bauteilflaechen_file_inputs: this.$xstate.fromPromise(
            this.$methode.actor_tranform_bauteilflaechen_file_inputs
          ),
          actor_tranform_aufbauten_file_inputs: this.$xstate.fromPromise(
            this.$methode.actor_tranform_aufbauten_file_inputs
          ),
        },
      })
      .createMachine({
        context: react_calculation_materials_context.parse(context ?? {}),
        output: ({
          context,
        }: {
