...
export class FgSpinnerMachineMethodeService extends FgBaseService {
  ...
  @boundMethod
  public guard_force_hide_is_true({ event }: { event: any }) {
    const valid_event = EventFgSpinnerHideParser.parse(event);
    return valid_event.payload.force;
  }

  @boundMethod
  public guard_force_show_is_true({ event }: { event: any }) {
  ...
  @boundMethod
  public delay_min_show_time({ context }: { context: ContextFgSpinner }) {
  ...
  @boundMethod
  public delay_before_shown({ context }: { context: ContextFgSpinner }) {
