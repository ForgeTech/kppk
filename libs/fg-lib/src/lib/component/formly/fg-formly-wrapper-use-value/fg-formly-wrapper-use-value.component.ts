...
@Component({
  selector: 'fg-formly-wrapper-use-value',
  templateUrl: './fg-formly-wrapper-use-value.component.html',
  styleUrls: ['./fg-formly-wrapper-use-value.component.scss'],
})
export class FgFormlyWrapperUseValueComponent extends FieldWrapper implements OnDestroy, AfterViewInit {
  ...
  public ngAfterViewInit(): void {
    if (this?.props['useValueName']) {
      this.useCrtlName = this.props['useValueName'];
    } else {
      this.useCrtlName = 'use'.concat(this.upperCaseFirstLetter(this.key.toString()));
    }

    const useCrtlValue = this.options.formState.model[this.useCrtlName]
      ? this.options.formState.model[this.useCrtlName]
      : false;
