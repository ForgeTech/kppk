          ...
          <div class="relative h-72 pb-[100px]">
            ...
            <table class="table-legend absolute bottom-0 right-0 w-[150px]">
              @for( item of this.graph_data_s().data; track $index; let i = $index ) {
                  <tr class="text-xs" [ngStyle]="{'background-color': graph_data_s().color_scheme.domain[ i ]}">
                    <td>{{ item.name }} </td>
...
export class KppkReactResultsPieChartCo2PhasesComponent {
  ...
  protected graph_data_s = computed( () => {
    ...
    const results_chart = [];
    if( this.material_s().co2_sum.value + this.material_s().co2_sum_transport.value ) {
      results_chart.push({
        name: 'Materialien',
        value: this.material_s().co2_sum.value + this.material_s().co2_sum_transport.value//this.data_s().absorbing.sum.value
      });
    ...
    if( this.data_s().form_construction_site.value.results.co2_supply.value ) {
      results_chart.push({
        name: 'Baustelle',
        value: this.data_s().form_construction_site.value.results.co2_supply.value
      });
    ...
    if(  this.data_s().form_demolish_disposal.value.results.consumption_co2_sum.value ) {
      results_chart.push({
        name: 'Abbruch',
        value: this.data_s().form_demolish_disposal.value.results.consumption_co2_sum.value
      });
      color_scheme.domain.push(this.$colors.demolish_disposal)
    }
    const form_excavation_pit_value = this.data_s().form_excavation_pit.value.results.excavation.co2_transport.value
      + this.data_s().form_excavation_pit.value.results.excavation_pit_security.co2_creation.value 
      + this.data_s().form_excavation_pit.value.results.excavation_pit_security.co2_transport.value;
    ...
    if(  this.data_s().form_heating_system.value.results.calc_usage_co2.value ) {
      results_chart.push({
        name: 'Gebäudebetrieb',
        value: this.data_s().form_heating_system.value.results.calc_usage_co2.value * 1000
      });
