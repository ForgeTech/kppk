...
@Component({
  ...
  imports: [CommonModule, FormlyModule, MatIconModule, MatButtonModule, MatTooltipModule],
  template: `
          ...
          <div class="flex flex-row">
            ...
            <div class="relative flex w-16">
              <button 
                ...
                color="warn" 
                type="button" 
                [matTooltip]="(field.props && field.props['delete-btn-tooltip'] || '')"
                (click)="remove(i)"
                >
...
export class KppkEnergyUsageYearlyArrayTypeComponent extends FieldArrayType {
  public add_construction_site_item() {
    this.add( this.field.fieldGroup?.length, construction_site_energy_usage_year_energy_usage_item_parser.parse({}) )
  }
}