...
import { FgBaseService, FgEnvironmentConfigInterface, FgEnvironmentService, FgImmutableService } from '@kppk/fg-lib-new';
...
export type CONFIG_ENVIRONMENT = FgEnvironmentConfigInterface<AppConfigInterface>;
...
export class ReactMainV2Service extends FgBaseService implements OnDestroy {
  ...
  public context_react_main = ({ input }: { input: CONFIG_ENVIRONMENT }) => {
    ...
    this.$log.warn('TODO: Add parser for ENVIRONMENT_CONFIG/CONTEXT_REACT_MAIN_V2 input')
    return context;
  }
  
  public input_react_init_actor = ({ context }: { context: CONTEXT_REACT_MAIN_V2 }) => {
  ...
  public input_react_running_actor = ({ context, event }: { context: CONTEXT_REACT_MAIN_V2, event: any}) => {
