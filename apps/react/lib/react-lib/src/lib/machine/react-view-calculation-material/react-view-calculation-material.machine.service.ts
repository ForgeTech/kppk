...
import { react_calculation_materials_context, REACT_CALCULATION_MATERIALS_CONTEXT, react_calculation_materials_output, REACT_CALCULATION_MATERIALS_OUTPUT } from '../../types';
...
export class ReactCalculationMaterialService extends FgBaseService {
  ...
  public get_machine( context?: REACT_CALCULATION_MATERIALS_CONTEXT ) {
    return this.$xstate.setup({
      ...
      output: ({ context }: { context: REACT_CALCULATION_MATERIALS_CONTEXT }) => {
        const result = react_calculation_materials_output.parse(context?.output);
        return result;
      },
