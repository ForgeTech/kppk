...
import { ReactViewCalculationMaterialMachineActorService } from "./react-view-calculation-material.machine.actor.service";
import { test_machine } from "../../testing";
import { EventFrom, SnapshotFrom, } from "xstate";

describe( 'ReactViewCalculationMaterialMachineActorService', () => {
    ...
    let $service: ReactViewCalculationMaterialMachineActorService;
    beforeEach( () => {
        TestBed.configureTestingModule({
            providers: [
                provideExperimentalZonelessChangeDetection(),
                importProvidersFrom(LoggerTestingModule),
                { provide: FgAuthLocalMachineService, useValue: fg_auth_local_service_mock },
                { provide: FgXstateService, useValue: fg_xstate_service_mock },
                { provide: FgEnvironmentService, useValue: fg_environment_service_mock_development_enabled },
                ReactViewCalculationMaterialMachineActorService
            ]
        });
        $service = TestBed.inject(ReactViewCalculationMaterialMachineActorService);
        jest.clearAllMocks();
        jest.clearAllTimers();
    });
    ...
    describe('> FgEnvironmentService > development > enabled: true', () => {
        ...
        it('MEMBER: event$ > emit events emitted by test_machine', done  => {
            const subscribtion = $service.event$.subscribe( value => {
                const snapshot =  $service['actor'].getSnapshot() as unknown as SnapshotFrom<typeof test_machine>
                switch(snapshot.value) {
                    case 'START':
                        expect( value ).toEqual({"type": "test_event", context: { value: 'start'}});
                    break;
                    case 'MIDDLE':
                        expect( value ).toEqual({"type": "test_event", context: { value: 'middle'}});
                    break;
                    case 'END':
                        expect( value ).toEqual({"type": "test_event", context: { value: 'end'}});
            ...
            $service.start()
            const event = {type: 'proceed'} as unknown as EventFrom<typeof $service['machine']>;
            $service.send( event );
            $service.send(event);
        });
        it('MEMBER: eventS > emits events emitted by test_machine', ()  => {
            ...
            $service.start()
            expect( $service.eventsS()).toEqual({"type": "test_event", context: { value: 'start'}});
            $service.send(event);
            expect( $service.eventsS()).toEqual({"type": "test_event", context: { value: 'middle'}});
            $service.send(event);
            expect( $service.eventsS()).toEqual({"type": "test_event", context: { value: 'end'}});
        });
        ...
        it('METHODE: create_from_config > setups new actor from config', () => {
            const state_subscription_spy = jest.spyOn($service['state_subscription'], 'unsubscribe');
            const events_subscription_spy = jest.spyOn($service['events_subscription'], 'unsubscribe');
        ...
        beforeEach( () => {
            ...
            TestBed.configureTestingModule({
                providers: [
                    ...
                    { provide: FgAuthLocalMachineService, useValue: fg_auth_local_service_mock },
                    { provide: FgXstateService, useValue: fg_xstate_service_mock },
                    { provide: FgEnvironmentService, useValue: fg_environment_service_mock_development_disabled },
                    ReactViewCalculationMaterialMachineActorService
                ]
            });
            $service = TestBed.inject(ReactViewCalculationMaterialMachineActorService);
