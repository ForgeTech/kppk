import { FG_FORM_CONTAINER_VILLAGE_CONTEXT } from "./kppk-react-calculation.machine.types";
...
export const calculate_energy_week = ( amount: UNIT_PIECES, energy_week: UNIT_KWH_WEEK): UNIT_KWH_WEEK => {
    const value = amount.value * energy_week.value;
    return unit_kilowatthours_week_parser.parse({ value });
}

export const calculate_energy_month = ( energy_week: UNIT_KWH_WEEK ): UNIT_KWH_MONTH => {
    const value = energy_week.value * 4;
    return unit_kilowatthours_month_parser.parse({ value });
}

export const calculate_energy = ( usage: UNIT_MONTH, energy_month: UNIT_KWH_MONTH ): UNIT_KWH => {
    const value = usage.value * energy_month.value;
    return unit_kilowatthours_parser.parse({ value });
}

export const calculate_energy_co2 = ( energy: UNIT_KWH, co2_equ: UNIT_KGCO2_KWH ): UNIT_KGCO2=> {
...
export const calculate_container_energy_results = (
    ...
    const result = Object.keys(container_type).map( key => {
        const energy_week = calculate_energy_week( container_values.amount, container_type[ key as keyof typeof container_type])
...
export const calculate_container_energy_sum = ( container_items: CONTAINER_VILLAGE_ENERGY_RESULT_ITEM[] ): UNIT_KWH => {
...
export const calculate_container_co2_sum = ( container_items: CONTAINER_VILLAGE_ENERGY_RESULT_ITEM[] ): UNIT_KGCO2 => {
    const value = container_items.reduce( (sum, item) => {
...
export const calculate_distance_co2 = ( distance: UNIT_KM, amount: UNIT_PIECES, co2_equ: UNIT_GCO2_KM ): UNIT_KGCO2 => {
...
export const calculate_container_village_results = ( 
  ...
  const energy_type = material_co2_equ_item_parser.parse(data.material_co2_equ.find( item => {
    return item.material.value === form_container_village.value.setting.energy_usage_power_type.value
  }));
  const energy_type_co2 = unit_kilogramco2_kilowatthour_parser.parse(energy_type.co2_equ);

  // Calculate energy co2
  const container_results = Object.keys( form_container_village.value.container ).forEach( key => {
    const container_data = form_container_village.value.container[ key as keyof typeof  form_container_village.value.container ];
    const container_type = data.container_village[ key as keyof typeof data.container_village ];
    const container_usage_results = calculate_container_energy_results( container_data, container_type, energy_type_co2);
    const container_energy_sum = calculate_container_energy_sum( container_usage_results );
    const container_co2_sum = calculate_container_co2_sum( container_usage_results );
    // add container co2
  ...
  // Calculate energy co2
  const distance_co2_equ = unit_gramco2_kilometer_parser.parse(data.truck.find( item => {
    return item.category.includes('common')
  })?.co2_consumption);
  result.distance_co2 = calculate_distance_co2( form_container_village.value.setting.distance, container_count, distance_co2_equ); 
