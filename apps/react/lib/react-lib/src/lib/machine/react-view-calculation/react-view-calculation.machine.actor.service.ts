...
import { ReactViewCalculationService } from './react-view-calculation.machine.service';
...
export class ReactViewCalculationMachineActorService extends FgBaseService implements OnDestroy {
  protected $env = inject(FgEnvironmentService);
  protected $xstate = inject(FgXstateService);
  protected $machine = inject(ReactViewCalculationService);
  ...
  public create_from_config( config: ActorOptions<ActorLogicFrom<any>> ) {
    if(this.is_runningS()) {
      this.$log?.warn( 'WARNING: ReactViewCalculationMachineActorService > create_from_with_config' );
