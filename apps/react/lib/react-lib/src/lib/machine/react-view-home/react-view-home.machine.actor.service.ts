...
import { ReactViewHomeMachineService } from './react-view-home.machine.service';
...
export class ReactViewHomeMachineActorService extends FgBaseService implements OnDestroy {
  protected $env = inject(FgEnvironmentService);
  protected $xstate = inject(FgXstateService);
  protected $machine = inject(ReactViewHomeMachineService);
  ...
  public create_from_config( config: ActorOptions<ActorLogicFrom<any>> ) {
    if(this.is_runningS()) {
      this.$log?.warn( 'WARNING: ReactInitMachineActorService > create_from_with_config' );
