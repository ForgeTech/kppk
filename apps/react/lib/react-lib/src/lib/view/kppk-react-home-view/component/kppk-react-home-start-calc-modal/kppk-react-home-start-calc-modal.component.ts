...
export class KppkReactHomeStartCalcModalComponent extends KppkReactBaseComponent {
  ...
  protected aufbauten_file = new FormControl( undefined, [Validators.required, FileValidator.maxContentSize(this.maxSize)])
  protected bauteilflaechen_file = new FormControl( undefined, [Validators.required, FileValidator.maxContentSize(this.maxSize)])
  ...
  protected aufbauten_file_change_s = toSignal(this.aufbauten_file.valueChanges, {initialValue: undefined})
  protected bauteilflaechen_file_change_s = toSignal(this.bauteilflaechen_file.valueChanges, {initialValue: undefined})
  protected heating_system_file_change_s = toSignal(this.heating_system_file.valueChanges, {initialValue: undefined})
  ...
  protected bauteilflaechen_file_change_e = effect( () => {
    // console.log('>>>>>>>>>>FILE>>>>>>>>')
    const fileInput = this.bauteilflaechen_file_change_s() as undefined | FileInput;
    if(fileInput?.files) {
  ...
  protected heating_system_file_change_e = effect( () => {
    ...
    const fileInput = this.heating_system_file_change_s() as undefined | FileInput;
    if(fileInput?.files) {
  ...
  public handle_rose_file( file: File ) {
    file.arrayBuffer().then( (buffer: ArrayBuffer) => {
      ...
      pdf2array( buffer, options ).then( pdfData => {
        ...
        let rose_table_system_row: ROSE_TABLE_SYSTEM_ROW | undefined = undefined;
        ...
        pdfData.forEach( (row, index) => {
          ...
          if( rose_table_section_headline_count === 2 && rose_table_system_row === undefined) {
            // Find system row
            try {
          ...
          if( rose_table_section_headline_count === 2 && rose_table_system_row && found_matching_row === undefined ) {
            // Find value row
            try {
        ...
        try {
          found_matching_row = (found_matching_row as unknown as ROSE_TABLE_ROW_TYPE);
          ...
          for( let index = 0; index < rose_table_system_row!.length; index++){
            ...
            else if(system_value.startsWith('district') || system_value.startsWith('fernwärme')) {
              rose_data_raw.district = unit_tonco2_xyear_parser.parse({ value })
            }
            else if(system_value.startsWith('air-water') || system_value.startsWith('luft-wasser')) {
              rose_data_raw.air_water = unit_tonco2_xyear_parser.parse({ value })
            }
            else if(system_value.startsWith('geothermal') || system_value.startsWith('sole-wasser')) {
              rose_data_raw.geothermal = unit_tonco2_xyear_parser.parse({ value })
            }
  ...
  const config = {
    ...
    // comments: false,
    // step: undefined,
    complete: (result:any) => this.transform_aufbauten_data(result.data as [ string[] ] ),
    error: ( error: any) => {},
    // download: false,
  ...
  public handle_bauteilflaechen_file(file: File ) {
    const config = {
      ...
      // comments: false,
      // step: undefined,
      complete: (result: any) => this.transform_bauteilflaechen_data(result.data as [ string[] ] )
      // error: undefined,
      // download: false,
      // downloadRequestHeaders: undefined,
      // downloadRequestBody: undefined,
