...
import { ReactAdminToolbarMachineActorService, ReactInitMachineActorService } from '../../machine';
...
export class KppkAdminToolbarComponent  {
  ...
  protected xstate_is_onS = computed( () => {
    return this.$actor_admin_toolbar.stateS()?.matches({'X_STATE': 'ON'}) ? true : false;
  });
  protected test_calculation_is_onS = computed( () => {
    return this.$actor_admin_toolbar.stateS()?.matches({'TEST_CALCULATION': 'ON'}) ? true : false;
  });
  ...
  protected toggle_xstate( event: MatSlideToggleChange ) {
    this.$actor_admin_toolbar.send({ type: 'react.admin_toolbox.event.x_state.toggle' });
  }
  protected toggle_test_calculation( event: MatSlideToggleChange ) {
    this.$actor_admin_toolbar?.send({ type: 'react.admin_toolbox.event.test_calculation.toggle' });
  }
}
