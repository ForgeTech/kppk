...
import { FgSpinnerMachineService } from '../machine';

@Injectable({
  providedIn: 'root'
})
export class KppkSpinnerActorService extends FgBaseService {
  ...
  public create_from_config( config: ActorOptions<ActorLogicFrom<any>> ) {
    if(this.is_runningS()) {
      this.$log?.warn( 'WARNING: KppkSpinnerActorService > create_from_with_config' );
