...
export class KppkAuthLocalActorService extends FgBaseService implements OnDestroy {
  ...
  public readonly eventsS = toSignal<EmittedFrom<typeof this.machine> | undefined>(this.EVENT$, { initialValue: undefined });
  protected events_subscription;

  protected STATE$ = new Subject<SnapshotFrom<typeof this.machine>>();
  public readonly state$ = this.STATE$.asObservable();
  public readonly stateS = toSignal<SnapshotFrom<typeof this.machine> | undefined>(this.STATE$, { initialValue: undefined });
  ...
  public create_from_config( config: ActorOptions<ActorLogicFrom<any>> ) {
    if(this.is_runningS()) {
      this.$log?.warn( 'WARNING: KppkAuthLocalActorService > create_from_with_config' );
      ...
      // Set existing inspect methode from old config
      // when not available od current config
      if(!config.inspect && this.config.inspect){
        config.inspect = this.config.inspect;
      }
      this.config = config;
      // Create actor
