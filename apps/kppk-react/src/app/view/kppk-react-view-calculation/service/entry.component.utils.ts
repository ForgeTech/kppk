...
export const transform_comma_seperated_numberstring_to_number = (
...
export const calculate_area_coefficient = (
  lathing_value1: {
    value: number | null;
    unit: string;
  },
  ...
  const result = aufbauten_data
    ...
    .map((row, index) => {
      const item = component_composition_step_1_parser.parse({
        ...
        column_2_2: {
          value: isNaN(transform_comma_seperated_numberstring_to_number(row[1]))
            ? null
            : transform_comma_seperated_numberstring_to_number(row[1]),
          unit: '',
        },
        layer: {
          value: row[2].trim(),
    ...
    .map((row) => {
      ...
      if (row.column_2_2.value === null) {
        ...
        // console.log( row )
        area_coefficient_object.area_coefficient = calculate_area_coefficient(
          row.lathing_thickness,
          row.lathing_spacing
        );
      } else {
        // console.log( 'else' )
...
export const transform_oi3 = (oi3_data: any[], bgf: number) => {
  return oi3_data
    ...
    .map((row_oi3) => {
      ...
      return material_oi3_step_2_parser.parse(
        Object.assign(row_oi3, gwp_total_object)
      );
    });
};
