...
export class KppkReactViewCalculationComponent {
  ...
  protected form_construction_site_model_s = computed(() => {
    const result =
      this.$actor_view_calculation.stateS()?.context.calculation
        ?.form_construction_site.value;
  ...
  protected send_event_to_calculation_machine(event: any) {
    this.$actor_view_calculation?.send(event);
  }
