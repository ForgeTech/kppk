...
import { test_environment, KppkAuthLocalActorService, KppkRegisterIconsService, KppkSpinnerActorService } from '@kppk/react-lib';
import { provideRouter, RouterModule } from '@angular/router';
import { provideAutoSpy } from 'jest-auto-spies';
import { FG_ENVIRONMENT, FgEnvironmentService, FgStorageNgxCookieService } from '@kppk/fg-lib-new';
...
describe('AppService', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
       ...
       providers: [
        ...
        { provide: KppkAuthLocalActorService, useValue: KppkAuthLocalActorMock },
        AppService,
       ]
    });
    jest.resetAllMocks();
  ...
  describe('>> on initialization and AFTER promise $app_ref.whenStable resolves', () => {
    it('After promise $app_ref.whenStable resolved hide event is sent to $spinner and app_readyS() is true', async () => {
      ...
      const event = { type: "fg.spinner.event.hide", payload: { force: false }};
      expect($service['$spinner']['send']).toHaveBeenCalledWith(event);
